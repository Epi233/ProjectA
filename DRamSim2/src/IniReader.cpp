		...
		for (size_t i = 0; configMap[i].variablePtr != nullptr; i++)
		{
			if (configMap[i].parameterType == type)
			{
				...
				switch (configMap[i].variableType)
				{
					...
					case UINT: visDataOut << *static_cast<unsigned *>(configMap[i].variablePtr);
						break;
					case UINT64: visDataOut << *static_cast<uint64_t *>(configMap[i].variablePtr);
						break;
					case FLOAT: visDataOut << *static_cast<float *>(configMap[i].variablePtr);
						break;
					case STRING: visDataOut << *static_cast<string *>(configMap[i].variablePtr);
						break;
					case BOOL: if (*static_cast<bool *>(configMap[i].variablePtr))
						{
							visDataOut << "true";
						}
						else
	...
	void IniReader::SetKey(string key, string valueString, bool isSystemParam, size_t lineNumber)
	{
		...
		for (i = 0; configMap[i].variablePtr != nullptr; i++)
		{
			...
			if (key.compare(configMap[i].iniKey) == 0)
			{
				...
				{
						//parse and set each type of variable
					case UINT: if ((iss >> dec >> intValue).fail())
						{
						...
						*static_cast<unsigned *>(configMap[i].variablePtr) = intValue;
						...
						break;
					case UINT64: if ((iss >> dec >> int64Value).fail())
						{
							ERROR("could not parse line "<<lineNumber<<" (non-numeric value '"<<valueString<<"')?");
						}
						*static_cast<uint64_t *>(configMap[i].variablePtr) = int64Value;
						...
						break;
					case FLOAT: if ((iss >> dec >> floatValue).fail())
						{
							ERROR("could not parse line "<<lineNumber<<" (non-numeric value '"<<valueString<<"')?");
						}
						*static_cast<float *>(configMap[i].variablePtr) = floatValue;
						...
						break;
					case STRING: *static_cast<string *>(configMap[i].variablePtr) = string(valueString);
						if (DEBUG_INI_READER)
						{
						...
						break;
					case BOOL: if (valueString == "true" || valueString == "1")
						{
							*static_cast<bool *>(configMap[i].variablePtr) = true;
						}
						else
						{
	...
	void IniReader::ReadIniFile(string filename, bool isSystemFile)
	{
		...
		if (iniFile.is_open())
		{
			while (!iniFile.eof())
			{
				...
				SetKey(key, valueString, isSystemFile, lineNumber);
			...
			if (!configMap[i].wasSet)
			{
				...
				switch (configMap[i].variableType)
				{
					...
					case FLOAT:
						...
						return false;
						break;
					case BOOL: *static_cast<bool *>(configMap[i].variablePtr) = false;
