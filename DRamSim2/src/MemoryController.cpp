	...
	, refreshRank(0)
{
	...
	{
		refreshCountdown.push_back(static_cast<int>((REFRESH_PERIOD / tCK) / NUM_RANKS) * (i + 1));
	}
}
			...
			if (bankStates[i][j].stateChangeCountdown > 0)
			{
				...
				if (bankStates[i][j].stateChangeCountdown == 0)
				{
					switch (bankStates[i][j].lastCommand)
					{
						...
						case WRITE_P:
						case READ_P: bankStates[i][j].currentBankState = Precharging;
							bankStates[i][j].lastCommand = PRECHARGE;
							bankStates[i][j].stateChangeCountdown = tRP;
							break;

						case REFRESH:
						case PRECHARGE: bankStates[i][j].currentBankState = Idle;
							break;
						default: break;
					}
				}
	...
	if (commandQueue.pop(&poppedBusPacket))
	{
		...
		switch (poppedBusPacket->busPacketType)
		{
			...
			case READ:
				...
				break;
			case WRITE_P:
			case WRITE: if (poppedBusPacket->busPacketType == WRITE_P)
				{
			...
			case ACTIVATE:
				...
				break;
			case PRECHARGE: bankStates[rank][bank].currentBankState = Precharging;
	...
	for (size_t i = 0; i < transactionQueue.size(); i++)
	{
		...
		if (commandQueue.hasRoomFor(2, newTransactionRank, newTransactionBank))
		{
			...
			BusPacket* ACTcommand = new BusPacket(ACTIVATE, transaction->address,
			                                      ...
			                                      newTransactionBank, nullptr, dramsim_log);
		...
		// no room, do nothing this cycle//PRINT( "== Warning - No room in command queue" << endl;
	}
...
void MemoryController::printStats(bool finalStats)
{
	...
	double secondsThisEpoch = static_cast<double>(cyclesElapsed) * tCK * 1E-9;
	...
	for (size_t i = 0; i < NUM_RANKS; i++)
	{
		for (size_t j = 0; j < NUM_BANKS; j++)
		{
			bandwidth[SEQUENTIAL(i, j)] = ((static_cast<double>(totalReadsPerBank[SEQUENTIAL(i, j)] + totalWritesPerBank[SEQUENTIAL(i, j)]) * static_cast<double>(bytesPerTransaction)) / (1024.0 * 1024.0 * 1024.0)) / secondsThisEpoch;
			averageLatency[SEQUENTIAL(i, j)] = (static_cast<float>(totalEpochLatency[SEQUENTIAL(i, j)]) / static_cast<float>(totalReadsPerBank[SEQUENTIAL(i, j)])) * tCK;
	...
	for (size_t r = 0; r < NUM_RANKS; r++)
	{
		...
		backgroundPower[r] = (static_cast<double>(backgroundEnergy[r]) / static_cast<double>(cyclesElapsed)) * Vdd / 1000.0;
		burstPower[r] = (static_cast<double>(burstEnergy[r]) / static_cast<double>(cyclesElapsed)) * Vdd / 1000.0;
		refreshPower[r] = (static_cast<double>(refreshEnergy[r]) / static_cast<double>(cyclesElapsed)) * Vdd / 1000.0;
		actprePower[r] = (static_cast<double>(actpreEnergy[r]) / static_cast<double>(cyclesElapsed)) * Vdd / 1000.0;
		averagePower[r] = ((backgroundEnergy[r] + burstEnergy[r] + refreshEnergy[r] + actpreEnergy[r]) / static_cast<double>(cyclesElapsed)) * Vdd / 1000.0;
